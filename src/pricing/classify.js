...
function pickRecommendedDiscountPct({
  ...
  if (hardCap < 5) return { recommended_discount_pct: null, deep_clearance_review: false };

  let target = 5;
  if (stock_class === "HIGH") {
    target = demand_trend === "slowing" ? 10 : 5;
  } else if (stock_class === "OVERSTOCK") {
    target = demand_trend === "slowing" || demand_trend === "insufficient_data" ? 15 : 10;
  ...
  // Snap down to largest recommend_band ≤ min(target, hardCap)
...
function classifyPricingAction(row, thresholds) {
  ...
  const floor = minimumMarginPrice(cost, t.min_gross_margin_pct);
  const costFloor = round2(cost);
  const commercialFloor = Math.max(costFloor, floor == null ? costFloor : floor);
  const maxSafe = maximumSafeDiscountPct(price, commercialFloor);

  // Price increase candidates
  const sold30 = Number(row.units_sold_30d) || 0;
  const trend = row.demand_trend || "insufficient_data";
  const gm = row.unit_gm_pct;
  const canIncrease =
    sold30 >= t.min_units_30d_for_price_increase &&
    (trend === "accelerating" || trend === "stable") &&
    PROTECT_STOCK.has(stock) &&
    gm != null &&
    gm >= t.min_gross_margin_pct;

  if (canIncrease) {
    const tests = (t.increase_steps || []).map((inc) =>
      simulateIncrease(price, cost, inc)
    );
    return {
  ...
  if (PROTECT_STOCK.has(stock) && sold30 > 0) {
    return {
  ...
  if (PROTECT_STOCK.has(stock) && sold30 === 0) {
    // Low stock + no demand → do not discount, monitor
    return {
      recommendation: "PROTECT_PRICE",
      recommended_discount_pct: null,
      recommended_price: null,
      ...
      maximum_safe_discount_pct: maxSafe,
      confidence: "low",
      note: "low_stock_without_discount",
    };
  }
  ...
  if (MODEST_STOCK.has(stock)) {
    ...
    const sim = simulateDiscount(price, cost, pick.recommended_discount_pct);
    return {
  ...
  if (CLEAR_STOCK.has(stock)) {
    ...
    return {
      ...
      ...pick,
      confidence:
        value >= t.min_inventory_value_for_clearance && stock === "NO_DEMAND"
          ? "high"
          : "medium",
    };
  }
  ...
  if (PROMO_STOCK.has(stock)) {
    ...
    return {
      ...
      maximum_safe_discount_pct: maxSafe,
      ...pick,
      confidence: stock === "NO_RECENT_DEMAND" ? "medium" : "medium",
    };
  }
