...
function extractWebhookAttribution(order) {
  ...
  return {
    ...
    first_campaign: first.campaign || first.campaign_id || notes.wa_ft_campaign || "",
    first_content: first.content || first.ad_id || notes.wa_ft_content || "",
    last_source: last.source || notes.wa_lt_source || "",
    last_campaign: last.campaign || last.campaign_id || notes.wa_lt_campaign || "",
    last_content: last.content || last.ad_id || notes.wa_lt_content || "",
    meta_click: Boolean(first.fbclid || first.fbc || last.fbclid || last.fbc || notes.wa_ft_fbclid)
      ? "Y"
      : "N",
    attribution_version: payload?.version || notes.wa_attr_version || "",
